    ...
    <form method="POST">
      <?php echo '<h4><a href="'.$jobs->jobs[$i]['source_url'].'" target="_blank"><img alt="'.$jobs->jobs[$i]['website_name'].
                 '" align="right" src="../design/static/images/favicons/'.$jobs->jobs[$i]['website_id'].'.ico" /></a> &nbsp; '
                  ...
                  .'<button class="btn btn-danger" name="delete" type="submit" onclick="return confirm(\'Are you sure you want to delete this job?\')">Delete</button>';
      
            if($jobs->jobs[$i]['show']==0)
                echo '<button class="btn btn-success" name="show" type="submit">Show</button>';
            else
                echo '<button class="btn btn-inverse" name="hide" type="submit">Hide</button>';
      
            echo '</h5></div></h4><input type="hidden" name="id_job" value="'.$jobs->jobs[$i]['id_job'].'">';

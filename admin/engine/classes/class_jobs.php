...
class jobs
{
     ...
     * @param type $id_job 
     */
    public function show_job($id_job)
    {
        $id_job = intval($id_job);
        db::query("UPDATE `jobs` SET `show`=1 WHERE `id_job`=$id_job");
    }
     ...
     * @param type $id_job 
     */
    public function hide_job($id_job)
    {
        $id_job = intval($id_job);
    ...
    {
        $id_job = intval($id_job);
        db::query("DELETE FROM `jobs` WHERE `id_job`=$id_job");
        db::query("DELETE FROM `jobs_categories` WHERE `id_job`=$id_job");
        db::query("DELETE FROM `jobs_job_types` WHERE `id_job`=$id_job");
    }
    ...
    public function get_jobs($limit,$skip=0)
    {
        ...
        else
        {
            ...
            $_SESSION['websites_admin'] = $this->create_website_checkboxes();
        ...
        else
        {
            ...
            $_SESSION['job_types_admin'] = $this->create_job_types_checkboxes();
        ...
        else
        {
            ...
            $_SESSION['categories_admin'] = $this->create_categories_checkboxes();
    ...
    public function get_websites()
    {
        ...
        $query = db::query("SELECT `id_website`,`name` FROM `websites` ORDER BY `order`");
        for($i = 0; $r = $query->fetch(); $i++)
        {
            ...
            if(!isset($_SESSION['websites_admin'][$r['id_website']]))
            {
                $list_of_websites[$i]['checked'] = 1;
                $_SESSION['websites_admin'][$r['id_website']] = 1; // adding the website to sessions
            }
            
            elseif($_SESSION['websites_admin'][$r['id_website']] == 1)
                $list_of_websites[$i]['checked'] = 1;
            else
    ...
    public function get_job_types()
    {
        ...
        for($i = 0; $r = $query->fetch(); $i++)
        {
            ...
            if(!isset($_SESSION['job_types_admin'][$r['id_job_type']]))
            {
                $list_of_job_types[$i]['checked'] = 1;
                $_SESSION['job_types_admin'][$r['id_job_type']] = 1; // adding the category to sessions
            }
            
            elseif($_SESSION['job_types_admin'][$r['id_job_type']] == 1)
                $list_of_job_types[$i]['checked'] = 1;
            else
    ...
    public function get_categories()
    {
        ...
        for($i = 0; $r = $query->fetch(); $i++)
        {
            ...
            $list_of_categories[$i]['name']        = $r['name'];
                ...
                // If there are no session with this category, its a new category, so making it for start as checked
            if(!isset($_SESSION['categories_admin'][$r['id_category']]))
            {
                $list_of_categories[$i]['checked'] = 1;
                $_SESSION['categories_admin'][$r['id_category']] = 1; // adding the category to sessions
            }
            
            elseif($_SESSION['categories_admin'][$r['id_category']] == 1)
                $list_of_categories[$i]['checked'] = 1;
            else
    ...
    public function create_website_checkboxes()
    {
        ...
        $query = db::query("SELECT `id_website` FROM `websites` ORDER BY `order`");
        for($i = 0; $r = $query->fetch(); $i++)
        {
                // Marking all of them as checked
    ...
    public function create_job_types_checkboxes()
    {
        ...
        $checked = array();
            ...
            // Gettigng all the websites
        $query = db::query("SELECT `id_job_type` FROM `job_types` ORDER BY `order`");
        for($i = 0; $r = $query->fetch(); $i++)
        {
                // Marking all of them as checked
    ...
    public function create_categories_checkboxes()
    {
        ...
        $checked = array();
            ...
            // Gettigng all the websites
        $query = db::query("SELECT `id_category` FROM `categories` ORDER BY `order`");
        for($i = 0; $r = $query->fetch(); $i++)
        {
                // Marking all of them as checked
    ...
    {
        foreach($_SESSION['websites_admin'] as $key => $value)
        {
            if(!isset($checked[$key]))
                $_SESSION['websites_admin'][$key] = 0;
            elseif($checked[$key][0]==1)
                $_SESSION['websites_admin'][$key] = 1;
        }
    }
    ...
    {
        foreach($_SESSION['job_types_admin'] as $key => $value)
        {
            if(!isset($checked[$key]))
                $_SESSION['job_types_admin'][$key] = 0;
            elseif($checked[$key][0]==1)
                $_SESSION['job_types_admin'][$key] = 1;
        }
    }
    ...
    {
        foreach($_SESSION['categories_admin'] as $key => $value)
        {
            if(!isset($checked[$key]))
                $_SESSION['categories_admin'][$key] = 0;
            elseif($checked[$key][0]==1)
